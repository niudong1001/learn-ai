...
- **频率学派**：特征是把需要推断的概率参数$\theta$视作**固定但未知的常数**，而样本$X$是随机的，重点是研究**样本空间**（需要不断做实验，概率计算都是针对样本的分布）。即可理解为：频率学派为事件本身建模，其认为数据都是在服从某个固定的参数值下产生的，需要做的就是不停的抽样来找到这个值，如事件A在独立重复试验中发生的频率趋于极限p，那么这个极限就是该事件的概率（只要做的实验足够多就可以找到它的真实概率）。
